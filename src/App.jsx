...
const samplePlaces = [
  ...
  },
  {
    ...
    location: 'Negombo',
    address: 'Negombo Lagoon, Negombo',
    distanceKm: 3.1,
    rating: 4.7,
    reviews: 96,
    price: 'From Rs. 3,000',
    ...
    openNow: true,
    tags: ['Outdoor', 'Sunset', 'Activity'],
    features: ['Guide included', 'Life jackets', 'Pre-booking', 'Couple sessions'],
  ...
  },
  {
    ...
    openNow: true,
    tags: ['Private cinema', 'Bookable', 'Indoor'],
    features: ['Private room', 'Air conditioned', 'Snacks available', 'Advance booking'],
  ...
  },
  {
    ...
    location: 'Negombo',
    address: 'Lagoon View Road, Negombo',
    distanceKm: 4.0,
    rating: 4.9,
    reviews: 211,
    price: 'From Rs. 1,500',
    ...
    openNow: true,
    tags: ['Quiet', 'View', 'Relax'],
    features: ['Outdoor seating', 'Drinks', 'Parking', 'Sunset view'],
  ...
  },
  {
    ...
    openNow: true,
    tags: ['Stay', 'Pool', 'Beach nearby'],
    features: ['Private room', 'Pool', 'Breakfast', 'Parking'],
  ...
  },
  {
    ...
    name: 'Little Italy',
    category: 'Eat',
    note: 'Pizza, pasta and an easy date-night mood',
    ...
    openNow: true,
    tags: ['Italian', 'Dinner', 'Casual'],
    features: ['Indoor seating', 'Takeaway', 'Parking', 'Desserts'],
    image: 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=1200&q=85',
  },
  {
    id: 7,
    name: 'Clay Date Studio',
    category: 'Do',
    note: 'Make something together and take it home',
    ...
    price: 'From Rs. 3,500',
    priceLevel: 2,
    time: 'Sessions until 8:00 PM',
    openNow: false,
    tags: ['Creative', 'Indoor', 'Workshop'],
    features: ['Materials included', 'Beginner friendly', 'Take-home piece', 'Pre-booking'],
  ...
  },
  {
    ...
    name: 'Cloud Spa',
    category: 'Relax',
    note: 'A calm wellness stop for your date plan',
    ...
    location: 'Negombo',
    address: 'St. Joseph Street, Negombo',
    distanceKm: 4.6,
    rating: 4.6,
    reviews: 103,
    price: 'From Rs. 4,000',
    ...
    openNow: true,
    tags: ['Wellness', 'Indoor', 'Relax'],
    features: ['Appointments', 'Air conditioned', 'Parking', 'Couple packages'],
    ...
    <article className="place-card phase2-card" onClick={() => onOpen(place.id)}>
      ...
      <div className="place-content">
        ...
        <div className="place-footer">
          ...
          <button
            className={added ? 'add-plan-button added' : 'add-plan-button'}
            onClick={(event) => {
              event.stopPropagation();
              onAdd(place.id);
            }}
    ...
    <main className="phase2-page">
      <section className="section-wrap explore-hero">
        <div>
          ...
          <p>Browse couple-friendly ideas around {location}. Keep it simple, save what you like, and add the best ones to your date plan.</p>
        </div>
        ...
        <div className="explore-search-box">
          ...
          <input
            ...
            placeholder="Search restaurants, activities, places..."
            aria-label="Search places"
          />
        </div>
      </section>

      <section className="section-wrap filter-panel" aria-label="Explore filters">
        <div className="filter-category-row">
          ...
          {categories.map(({ name }) => (
            <button
              key={name}
              className={activeCategory === name ? 'filter-chip active' : 'filter-chip'}
        ...
        <div className="filter-actions">
          <button className={openOnly ? 'filter-control active' : 'filter-control'} onClick={() => setOpenOnly(!openOnly)}>
      ...
      <section className="section-wrap explore-results">
        <div className="results-heading">
          <div>
            <span className="mini-label">{places.length} places found</span>
            <h2>{activeCategory === 'All' ? `Ideas around ${location}` : `${activeCategory} around ${location}`}</h2>
          </div>
        </div>
          ...
          <div className="place-grid explore-grid">
            {places.map((place) => (
              <PlaceCard
                ...
                saved={savedItems.includes(place.id)}
                onAdd={togglePlan}
                onSave={toggleSaved}
                onOpen={openPlace}
              />
    ...
    <main className="details-page">
      <section className="section-wrap detail-top-row">
        ...
        <div className="detail-top-actions">
          <button className="icon-text-button"><Share2 size={17} /> Share</button>
          <button className={saved ? 'icon-text-button saved' : 'icon-text-button'} onClick={() => onSave(place.id)}>
      ...
      <section className="section-wrap detail-hero-grid">
        <div className="detail-image-panel">
          ...
          <span className="detail-category">{place.category}</span>
        </div>
        ...
        <div className="detail-summary">
          ...
          <div className="detail-rating-line">
            <span className="rating large"><Star size={18} fill="currentColor" /> {place.rating.toFixed(1)}</span>
          ...
          <div className="detail-info-list">
            <div><MapPin size={18} /><span><strong>{place.address}</strong><small>{place.location}</small></span></div>
            <div><Clock3 size={18} /><span><strong>{place.time}</strong><small>{place.openNow ? 'Open now' : 'Check availability before going'}</small></span></div>
          </div>
          ...
          <div className="detail-booking-card">
            ...
            <button className={added ? 'primary-detail-button added' : 'primary-detail-button'} onClick={() => onAdd(place.id)}>
              {added ? <><Check size={18} /> Added to date plan</> : <><CalendarDays size={18} /> Add to date plan</>}
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="section-wrap detail-body-grid">
        <div className="detail-main-copy">
          <div className="detail-section-block">
            <span className="mini-label">Why couples like it</span>
            <h2>A simple idea for time together</h2>
            <p>{place.description}</p>
          </div>

          <div className="detail-section-block">
            <span className="mini-label">Good to know</span>
            <h2>What you can expect</h2>
            <div className="feature-grid">
              {place.features.map((feature) => (
                <div key={feature}><Check size={17} /><span>{feature}</span></div>
              ))}
            </div>
          </div>
        </div>
        ...
        <aside className="detail-side-card">
          <span className="mini-label">Plan this stop</span>
          <h3>Ready to add it?</h3>
          <p>Keep this place together with the rest of your date ideas.</p>
          <button className={added ? 'side-plan-button added' : 'side-plan-button'} onClick={() => onAdd(place.id)}>
            <CalendarDays size={18} /> {added ? 'Remove from plan' : 'Add to date plan'}
          </button>
          <button className="contact-button"><Phone size={18} /> Contact place</button>
        </aside>
      </section>
    </main>
  );
...
function App() {
  ...
  const homePlaces = useMemo(() => {
    if (activeCategory === 'All') return samplePlaces.slice(0, 5);
    return samplePlaces.filter((place) => place.category === activeCategory).slice(0, 5);
  }, [activeCategory]);
  ...
  const explorePlaces = useMemo(() => {
    ...
    const filtered = samplePlaces.filter((place) => {
      ...
      const searchableText = `${place.name} ${place.category} ${place.note} ${place.tags.join(' ')}`.toLowerCase();
  ...
  const openExplore = (category = activeCategory) => {
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <div className="desktop-nav">
            ...
            <button className="desktop-nav-link">Date Plan {planItems.length > 0 && <span>{planItems.length}</span>}</button>
          </div>
        ...
        <main>
          ...
          <section className="section-wrap category-section">
            ...
            <div className="category-grid">
              {categories.map(({ name, icon: Icon, helper }) => (
                <button key={name} className="category-tile" onClick={() => openExplore(name)}>
          ...
          <section className="section-wrap places-section">
            <div className="section-heading">
              ...
              <button className="text-button" onClick={() => openExplore(activeCategory)}>Explore all</button>
            </div>
      ...
      {view === 'details' && selectedPlace && (
        <PlaceDetails
          place={selectedPlace}
          added={planItems.includes(selectedPlace.id)}
          saved={savedItems.includes(selectedPlace.id)}
          onBack={() => openExplore(activeCategory)}
          onAdd={togglePlan}
          onSave={toggleSaved}
        />
      )}
