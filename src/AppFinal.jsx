      ...
      <div className="place-content">
        ...
        <div className="place-footer">
          ...
          <button
            ...
            onClick={(event) => { event.stopPropagation(); onAdd(place.id); }}
          >
      ...
      <section className="section-wrap explore-results">
        ...
        {places.length ? (
          ...
          <div className="empty-state">
            ...
            <p>Try another search/category or switch back to Negombo while Twonara grows to more areas.</p>
            <div className="empty-actions"><button onClick={() => { setQuery(''); setActiveCategory('All'); }}>Clear filters</button><button onClick={clearLocation}>Explore Negombo</button></div>
          </div>
        )}
      </section>
    </main>
...
function ReviewBox({ place, session, reviews, setReviews, onNeedLogin }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setMessage(error.message || 'Could not save review.');
    }
  };

  return (
    <section className="detail-section-block review-block">
      <span className="mini-label">Reviews</span>
      <h2>What people say</h2>
      <div className="review-list">
        {placeReviews.length === 0 ? <p className="muted-copy">No Twonara reviews yet. You can be the first after visiting.</p> : placeReviews.slice(0, 4).map((review) => (
          <article key={review.id}><div><strong>{review.userName || 'Twonara user'}</strong><span><Star size={14} fill="currentColor" /> {review.rating}</span></div><p>{review.comment}</p><small>{new Date(review.createdAt).toLocaleDateString()}</small></article>
        ))}
      </div>
      <form className="review-form" onSubmit={submit}>
        <span>Your rating</span>
        <div className="star-picker">{[1, 2, 3, 4, 5].map((value) => <button key={value} type="button" className={rating >= value ? 'active' : ''} onClick={() => setRating(value)} aria-label={`${value} stars`}><Star size={20} fill={rating >= value ? 'currentColor' : 'none'} /></button>)}</div>
        <textarea rows="3" value={comment} onChange={(event) => setComment(event.target.value)} placeholder={session ? 'Share a helpful, respectful review...' : 'Log in to write a review'} />
        {message && <div className="form-message">{message}</div>}
        <button className="primary-small-button" type="submit">{session ? 'Post review' : 'Log in to review'}</button>
      </form>
    </section>
  );
}
  ...
  return (
    <main className="details-page">
      <section className="section-wrap detail-top-row">
        ...
        <div className="detail-top-actions"><button className="icon-text-button" onClick={sharePlace}><Share2 size={17} /> Share</button><button className={saved ? 'icon-text-button saved' : 'icon-text-button'} onClick={() => onSave(place.id)}><Heart size={17} fill={saved ? 'currentColor' : 'none'} /> {saved ? 'Saved' : 'Save'}</button></div>
      </section>
      ...
      <section className="section-wrap detail-body-grid">
        <div className="detail-main-copy">
          ...
          <ReviewBox place={place} session={session} reviews={reviews} setReviews={setReviews} onNeedLogin={onNeedLogin} />
        </div>
        <aside className="detail-side-card">
          ...
          <button className="contact-button" onClick={() => place.phone && (window.location.href = `tel:${place.phone}`)}><Phone size={18} /> Contact place</button>
...
function AppFinal() {
  ...
  const homePlaces = useMemo(() => {
    const sorted = [...locationPlaces].sort((a, b) => Number(Boolean(b.promoted)) - Number(Boolean(a.promoted)) || b.rating - a.rating);
  ...
  const openBusiness = () => {
    if (session?.role === 'business') go('business');
    else openAuth('business');
  };
    ...
    <div className="app-shell">
        ...
        <header className="topbar">
          <div className="topbar-inner">
            ...
            <div className="topbar-actions">
              ...
              <button className="profile-button" aria-label="Open profile" onClick={openProfile}><UserRound size={19} /></button>
            </div>
          </div>
        </header>
      )}
      ...
      {view === 'home' && (
        <main>
          ...
          <section className="section-wrap soft-cta"><div><span className="mini-label">For local businesses</span><h2>Have a place couples would love?</h2><p>Join Twonara, post your place and choose a Free, Premium or Featured listing.</p></div><button className="outline-button" onClick={openBusiness}>Post your place</button></section>
        </main>
      )}
      ...
      {view === 'auth' && <AuthPanel intentRole={authIntent} users={users} setUsers={setUsers} onSignedIn={handleSignedIn} onBack={goHome} />}
      ...
      {view !== 'business' && view !== 'admin' && (
        <nav className="mobile-nav" aria-label="Main navigation">
          ...
          <button className={view === 'saved' ? 'mobile-nav-item active' : 'mobile-nav-item'} onClick={openSaved}><Bookmark size={20} /><span>Saved{savedItems.length > 0 ? ` (${savedItems.length})` : ''}</span></button>
          <button className={view === 'profile' || view === 'auth' ? 'mobile-nav-item active' : 'mobile-nav-item'} onClick={openProfile}><UserRound size={20} /><span>Profile</span></button>
        </nav>
      )}
    </div>
  );
